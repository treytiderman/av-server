...
async function deleteGroup(group) {
    ...
    // Delete group
    db.data.groups = db.data.groups.filter(group => group !== group)
    emitter.emit('groups', getGroups())

    // Remove group from all user's group lists
    const newUsers = []
    getUsersAndPasswords().forEach(user => {
        user.groups = user.groups.filter(group => group !== group)
...
async function createUser(username, password, passwordConfirm, groups = []) {
    ...
    else if (!validPassword(password)) error = `error password ${password} NOT valid`
