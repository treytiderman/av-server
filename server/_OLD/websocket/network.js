...
if (os.type() === "Windows_NT") {
  ...
  // Module
  const netsh = require("../core/modules/netsh")
  ...
  ws_server.emitter.on("/network/v1", async (ws, req) => {
    if (req.event === "nics") {
      ...
      ws_server.eventAll(`/network/v1`, "nics", output)
    }
    else if (req.event === "dhcp/ip") {
      ws_server.subscribe(ws, `/network/v1`)
      const output = await netsh.setDhcpIp(req.body.nic)
      ws_server.eventAll(`/network/v1`, "dhcp/ip", {ip: output, nic: req.body.nic})
    }
    else if (req.event === "dhcp/dns") {
      ws_server.subscribe(ws, `/network/v1`)
      const output = await netsh.setDhcpDns(req.body.nic)
      ws_server.eventAll(`/network/v1`, "dhcp/dns", {dns: output, nic: req.body.nic})
    }
    else if (req.event === "dhcp") {
      ws_server.subscribe(ws, `/network/v1`)
      const output = await netsh.setDhcp(req.body.nic)
      ws_server.eventAll(`/network/v1`, "dhcp", {...output, nic: req.body.nic})
    }
      ...
      ws_server.eventAll(`/network/v1`, "static/ip", {ip: output, nic: req.body.nic})
    }
      ...
      ws_server.eventAll(`/network/v1`, "static/add", {ip: output, nic: req.body.nic})
    }
      ...
      ws_server.eventAll(`/network/v1`, "static/dns", {dns: output, nic: req.body.nic})
    }
      ...
      ws_server.eventAll(`/network/v1`, "static", {...output, nic: req.body.nic})
    }
      ...
      ws_server.eventAll(`/network/v1`, "metric", output)
    }
    else if (req.event === "metric/auto") {
      ws_server.subscribe(ws, `/network/v1`)
      const output = await netsh.setAutoMetric(req.body.nic)
      ws_server.eventAll(`/network/v1`, "metric/auto", {metric: output, nic: req.body.nic})
    }
  })
